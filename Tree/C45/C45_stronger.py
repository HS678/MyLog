...
def dataProcess(filename, output_file):
    ...
    midu_index = data.columns.get_loc('密度')
    tang_index = data.columns.get_loc('含糖率')
    midu_ta = lianxu(dataset, midu_index)
    tang_ta = lianxu(dataset, tang_index)
    ...
    for column in data.columns[:-3]:  # 最后三列不需要转换
    ...
    # 将小于阈值的标记为0，大于等于阈值的标记为1
    data['密度'] = data['密度'].map(lambda m: 0 if m < midu_ta else 1)
    data['含糖率'] = data['含糖率'].map(lambda n: 0 if n < tang_ta else 1)
    ...
    with open(output_file, 'w') as file:
        for column in data.columns[:-3]:  # 最后一列是目标变量，不需要写入
            ...
            file.write("\n")
        # 写入密度和含糖率对应变换关系
        file.write(f"密度<{midu_ta:.3f}:\n")
        file.write("[0,是] [1,否]\n")
        file.write(f"含糖率<{tang_ta:.3f}:\n")
        file.write("[0,是] [1,否]\n")
...
def min_info_gain(dataset):
    numFeatures = len(dataset[0]) -1
    baseEnt = cal_entropy(dataset)
    sum = 0.0
    for i in range(numFeatures):
        featList1 = [example[i] for example in dataset]
        uniqueVals1 = set(featList1)
        newEnt1 = 0.0
        # 计算根据当前特征划分后的熵和TV
        for value in uniqueVals1:
            subdataset1 = splitdataset(dataset, i, value)
            p1 = len(subdataset1) / float(len(dataset))
            newEnt1 += p1 * cal_entropy(subdataset1)
        infoGain1 = baseEnt - newEnt1
        sum += infoGain1
    sum = sum * 1.0 / numFeatures
    return sum


# 选择最好的划分特征，标准：信息增益比
def C45_chooseBestFeatureToSplit(dataset, mininfogain):
    numFeatures = len(dataset[0]) - 1   # 特征数量
    baseEnt = cal_entropy(dataset)   # 基础熵，原数据集的熵
    bestInfoGain_ratio = 0.0   # 最好的信息增益比
    ...
    for i in range(numFeatures):
        ...
        print(u"本次需要满足的最小信息增益为：%.3f,第%d个特征的信息增益为：%.3f,信息增益率为：%.3f" % (mininfogain, i, infoGain, infoGain_ratio))
        if infoGain > mininfogain and infoGain_ratio > bestInfoGain_ratio:
...
def C45_createTree(dataset, labels):
    ...
    mininfogain=min_info_gain(dataset)
    bestFeatIndex = C45_chooseBestFeatureToSplit(dataset,mininfogain)
    ...
    for value in featValues:
        ...
        C45Tree[bestFeatLabel][value] = C45_createTree(subDataset, subLabels)
    return C45Tree
...
if __name__ == '__main__':
    ...
    # 构建决策树
    C45Tree = C45_createTree(train_data, labels_train)

    print(C45Tree)

    # 计算并输出准确率
    accuracy_score = accuracy(C45Tree, labels_train, test_data)
